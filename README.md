...
- the stage3 installer on external USB target is skippable or simply fixable when updating a previous already installed Catalina or BigSur, because it requires an already "apple setup done" (to fix the apfs firmlinks issues) that is a root account for the APFS Data Volume
